...
/// направляющими линиями (замерено программно по альфа/цветовым каналам
/// исходного PNG — 2130×302, ищем пиксели с преобладанием синего внутри
/// рамки) есть полость, где и должны лежать трек с заполнением, а не
/// просто "по центру рамки". `cavity…Fraction` ниже — координаты этой
...
/// с ней).
///
/// Прошлые значения (0.232/0.695/0.117) были на глаз и заметно промахивались
/// мимо реальной полости — из-за этого жёлтая полоска либо не доходила до
/// скруглённых концов рамки, либо вылезала за синюю подложку. Плюс
/// `ProgressBarTrack`/`ProgressBarFill` сами были нарезаны с разным
/// количеством прозрачных полей по краям (6–81px у трека, 46–8px у
/// заполнения — не поровну!), так что даже при верных фракциях рамки два
/// слоя растягивались не в унисон. Оба спрайта перенарезаны вплотную по
/// непрозрачному контенту (без полей), поэтому теперь при растяжении в
/// один и тот же `cavityWidth × cavityHeight` они совпадают день в день.
struct OrnateProgressBar: View {
    let progress: Double

    /// Полость между направляющими линиями рамки: 19.3%–74.2% высоты рамки,
    /// с отступами ~7.9% слева/справа (где рамку огибают декоративные
    /// наконечники).
    private let cavityTopFraction: CGFloat = 0.193
    private let cavityBottomFraction: CGFloat = 0.742
    private let cavityInsetFraction: CGFloat = 0.079
